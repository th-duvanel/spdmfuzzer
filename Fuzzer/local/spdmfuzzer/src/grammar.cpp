...
{
    this->SPDM = 0x05;
    this->reqresCode = randomize(0, 255);
...
Version::Version() : responsePacket(RequestResponseCode["VERSION"], 1, 0, 0, 0)
{
    this->reserved = 0;
...
void* Version::serialize()
{
    ...
    buffer[4] = this->reserved;
    buffer[5] = this->entryCount;

    for(u8 i = 0 ; i < this->entryCount ; i++) {
        buffer[6 + (i * 2)] = this->entry[i] >> 8;
        buffer[7 + (i * 2)] = this->entry[i];
    }
...
// Mocked packets, in order of use:
...
u8 mockedCapabilities[] = "\x00\x00\x00\x01\x00\x00\x00\x0d\x05\x11\x61\x00\x00\x00\x00\x00" \
                          "\x00\xd6\xfb\x01\x00";

u8 mockedNegAlgorithms[] = "\x00\x00\x00\x01\x00\x00\x00\x31\x05\x11\xe3\x04\x00\x30\x00\x01" \
                           "\x00\x10\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" \
                           "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x20\x08\x00\x03\x20\x02" \
                           "\x00\x04\x20\x01\x00\x05\x20\x01\x00";

u8 mockedAlgorithms[] = "\x00\x00\x00\x01\x00\x00\x00\x35\x05\x11\x63\x04\x00\x34\x00\x01" \
                        "\x00\x02\x00\x00\x00\x10\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00" \
                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x20\x08" \
                        "\x00\x03\x20\x02\x00\x04\x20\x01\x00\x05\x20\x01\x00";

u8 mockedGetDigests[] = "\x00\x00\x00\x01\x00\x00\x00\x05\x05\x11\x81\x00\x00";

u8 mockedDigests[] = "\x00\x00\x00\x01\x00\x00\x00\x45\x05\x11\x01\x00\x03\xa0\x57\x50" \
